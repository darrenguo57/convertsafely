...
export default function VideoConverter() {
  ...
  const onDrop = useCallback((acceptedFiles: File[]) => {
    ...
    for (const file of acceptedFiles) {
      ...
      if (!validVideoTypes.includes(file.type) && !file.name.match(/\.(mp4|webm|ogg|mov|avi|mkv|flv)$/i)) {
        toast.error(`${file.name}: 不支持的视频格式`);
        continue;
      }
  ...
  const startConversion = useCallback(async () => {
    ...
    try {
      ...
      setStatus('completed');
      incrementUsage();
      toast.success(`成功转换 ${conversionResults.length} 个视频！`);
    } catch (err) {
    ...
    <div className="max-w-5xl mx-auto px-4 py-8">
      ...
      <div className="text-center mb-10">
        ...
        <p className="text-gray-600 dark:text-gray-400 max-w-2xl mx-auto">
          支持 MP4、WebM、OGG、MOV、AVI、MKV 等格式互转。使用 FFmpeg.wasm 在浏览器本地处理，保护您的隐私。
        </p>
      </div>
      ...
      {files.length === 0 && (
        ...
        >
          ...
          <p className="text-xl font-medium text-gray-900 dark:text-white mb-2">
            {isDragActive ? '释放以上传视频' : '拖拽视频到此处'}
          </p>
          <p className="text-gray-500 dark:text-gray-400 mb-4">
            或点击选择文件
          </p>
          <p className="text-sm text-gray-400 dark:text-gray-500">
            支持 MP4, WebM, OGG, MOV, AVI, MKV, FLV
          </p>
            ...
            <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6">
              ...
              <div className="mt-4 p-3 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
                <p className="text-sm text-gray-600 dark:text-gray-400">
                  视频转换可能需要较长时间，取决于文件大小和设备性能。请勿关闭页面。
                </p>
              </div>
            </div>
            ...
            {status === 'converting' && (
              ...
              >
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <span className="font-medium text-gray-900 dark:text-white">正在转换视频...</span>
                </div>
